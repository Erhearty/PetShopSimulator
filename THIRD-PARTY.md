...
>
> A fresh clone builds and runs without them, on the CC0 Kenney kits plus procedural
> fallbacks. To restore the full art, download each pack through Package Manager →
> My Assets, then run `./build.sh assets` (`./build.sh assets?` reports which packs are
> present).
...
project root (`PetShop.x86_64`, `PetShop_Data/`, `UnityPlayer.so`, …). Both were removed
